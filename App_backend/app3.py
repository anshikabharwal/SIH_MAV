...
def upload_file():
    ...
    if technique == 'eulerian':
        if params['mode'] == "0":
            ...
            result = process_video_Eulerian(vid=file_path, mode= mode, alpha=alpha,cutoff= cutoff,low= low,high=high,chromAttenuation= chromAttenuation,linearAttenuation=linearAttenuation)
        ...
        result = process_video_Phase(file_path, pyr_type, magnification_factor, f1, fh, fs,attenuate_other_freq,sigma)
