...
if __name__ == '__main__':
    ...
    while True:
        ...
        original_intensity.append(np.mean(final_img))
        magnified_intensity.append(np.mean(out))
        
        # Update the x and y data of the lines after 10 frames
        frame_counter += 1
        if frame_counter >= frame_check:
            # Update the x and y data of the lines
            original_line.set_data(range(len(original_intensity)), original_intensity)
